...
app.directive('autocomplete', function(\$compile, \$timeout, \$http) {
...
app.controller('DiagnosesController', [ '\$scope', '\$http' , '\$timeout', 'EncounterServices', 'recentVisitFactory',
function DiagnosesController(\$scope, \$http, \$timeout, EncounterServices, recentVisitFactory) {
  \$scope.alerts = [];
  \$scope.respuuid = [];
  var _selected;
...
\$timeout(function () {
  ...
  \$scope.addPriorDiagnoses = function() {
	...
	var promise = EncounterServices.getEncounter().then(function(response){
